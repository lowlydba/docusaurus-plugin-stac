...
/**
 * Storage-provider brand icons for `storage:schemes` entries (see
 * `StorageSchemes.tsx`). These are decorative reinforcement of the adjacent
 * text label, not the sole conveyor of information, so each icon is rendered
 * `aria-hidden` and non-focusable.
 *
 * `Aws`/`Azure`/`GoogleCloud` are traced from the "logos" icon set
 * (https://github.com/gilbarbara/logos, MIT/CC0), a set maintained
 * independently of `simple-icons` — which dropped its Amazon/Microsoft marks
 * after trademark takedown requests. `Generic` is adapted from Tabler Icons
 * (https://tabler.io/icons, MIT) and uses `currentColor` so callers can tint
 * it (we use a neutral gray) for schemes that aren't a recognized brand.
 */
...
function IconBase({
  ...
  return (
    <svg
      viewBox={viewBox}
      className={`stac-storage-schemes__icon ${className ?? ''}`.trim()}
      aria-hidden="true"
      focusable="false"
    >
      {children}
...
export function AwsIcon(): React.JSX.Element {
  return (
    <IconBase viewBox="0 0 256 153">
      <path
        fill="#252f3e"
...
export function AzureIcon(): React.JSX.Element {
  // Gradient IDs must be unique per rendered instance — `<defs>` IDs are
  // global to the document, and this icon can appear multiple times on one
  // page (one per Azure-detected scheme).
  const uid = React.useId();
  const gradA = `stac-azure-a-${uid}`;
  const gradB = `stac-azure-b-${uid}`;
  const gradC = `stac-azure-c-${uid}`;
  return (
    <IconBase viewBox="0 0 256 242">
      <defs>
        <linearGradient id={gradA} x1="58.972%" x2="37.191%" y1="7.411%" y2="103.762%">
          <stop offset="0%" stopColor="#114a8b" />
          <stop offset="100%" stopColor="#0669bc" />
        </linearGradient>
        <linearGradient id={gradB} x1="59.719%" x2="52.691%" y1="52.313%" y2="54.864%">
        ...
        <linearGradient id={gradC} x1="37.279%" x2="62.473%" y1="4.6%" y2="99.979%">
...
export function GoogleCloudIcon(): React.JSX.Element {
  return (
    <IconBase viewBox="0 0 256 206">
      <path
        fill="#ea4335"
...
/** Neutral fallback for schemes that aren't a recognized major provider. */
export function GenericStorageIcon(): React.JSX.Element {
  return (
    <IconBase viewBox="0 0 24 24" className="stac-storage-schemes__icon--generic">
      <path
        fill="none"
        stroke="currentColor"
        strokeLinecap="round"
...
export type StorageProviderKind = 'aws' | 'azure' | 'gcp' | 'generic';
